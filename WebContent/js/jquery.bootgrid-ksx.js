    ...
    response = {
        ...
        "bizContent": {
            ...
            "rows": {
                ...
                "consumeType": 3,
                "consumeAmount": 1,
                "examedUsername": "kuliniubi",
                "examName": "nba总决赛",
                "consumeTime": "2016-10-20 10:10:10",
                "consumeTypeName": "免登陆考试短信验证"
            },
            current: 1,
            rowCount: 10,
        }
    }
    所以在下面response参数中，使用了($.parseJSON(response)).bizContent
    */
        ...
        if (this.options.ajax)
        {
            ...
            var settings = {
                ...
                success: function(response)
                {
                    ...
                    {
                        response = ($.parseJSON(response)).bizContent;
                    }
    ...
    function renderColumnSelection(actions)
    {
        if (this.options.columnSelection && this.columns.length > 1)
        {
            var that = this,
                ...
                dropDown = $(tpl.actionDropDown.resolve(getParams.call(this, { content: icon + "自定义设置" }))),
    ...
    function renderPagination()
    {
        if (this.options.navigation !== 0)
        {
            ...
            if (this.rowCount !== -1 && paginationItems.length > 0)
            {
                ...
                renderPaginationItem.call(this, pagination, "first", '<i class="icons8-icon"></i>', "first")
                    ._bgEnableAria(current > 1);
                renderPaginationItem.call(this, pagination, "prev", '<i class="icons8-back-filled"></i>', "prev")
                    ._bgEnableAria(current > 1);
                ...
                renderPaginationItem.call(this, pagination, "next", '<i class="icons8-forward-filled"></i>', "next")
                    ._bgEnableAria(totalPages > current);
                renderPaginationItem.call(this, pagination, "last", '<i class="icons8-icon-3"></i>', "last")
                    ._bgEnableAria(totalPages > current);
    ...
    function renderRowCountSelection(actions)
    {
        ...
        if ($.isArray(rowCountList))
        {
            var css = this.options.css,
                tpl = this.options.templates,
                dropDown = $(tpl.actionDropDown.resolve(getParams.call(this, { content: "每页"+getText(this.rowCount)+"项" }))),
                    ...
                    { text: getText(value), uri: "#" + value })))
                        ...
                        .on("click" + namespace, menuItemSelector, function (e)
                        {
                            ...
                            if (newRowCount !== that.rowCount)
                            {
                                ...
                                $this.parents(menuSelector).find(menuTextSelector).eq(0).text("每页"+getText(newRowCount)+"项");
                                loadData.call(that);
                            }
                        });
    ...
    function renderRows(rows)
    {
        if (rows.length > 0)
        {
            ...
            that.element.find("thead input" + getCssSelector(that.options.css.selectBox))
    ...
    function registerRowEvents(tbody)
    {
        var that = this,
            ...
            selectLabelSelector = getCssSelector(this.options.css.selectLabel);

        if (this.selection)
        {
            tbody.off("click" + namespace, selectLabelSelector)
                .on("click" + namespace, selectLabelSelector, function(e)
                {
                    e.stopPropagation();

                    var $this = $(this).find("input"+selectBoxSelector),
    ...
    function renderTableHeader()
    {
                ...
                var sortOrder = that.sortDictionary[column.id],
                    iconCss = ((sorting && sortOrder && sortOrder === "asc") ? css.iconUp :
                        (sorting && sortOrder && sortOrder === "desc") ? css.iconDown :
                            ((column.id == "sender") ? css.iconOperation : "")),
        ...
        if (this.selection && this.options.multiSelect)
        {
            ...
            var selectLabelSelector = getCssSelector(css.selectLabel);

            headerRow.off("click" + namespace, selectLabelSelector)
                .on("click" + namespace, selectLabelSelector, function(e)
                {
                    e.stopPropagation();
                    var $this = $(this).find("input"+selectBoxSelector);
    ...
    Grid.defaults = {
        ...
        css: {
            ...
            dropDownMenu: "btn-group", // must be a unique class name or constellation of class names within the actionDropDown
            dropDownMenuItems: "dropdown-menu pull-right", // must be a unique class name or constellation of class names within the actionDropDown
            dropDownMenuText: "dropdown-text", // must be a unique class name or constellation of class names within the actionDropDown
            footer: "bootgrid-footer container-fluid",
            header: "bootgrid-header container-fluid",
            icon: "icon glyphicon",
            iconColumns: "icons8-settings",
            iconDown: "icons8-sort-filled",
            iconRefresh: "glyphicon-refresh",
            iconSearch: "glyphicon-search",
            iconUp: "icons8-sort-filled",
            iconOperation: "icons8-settings",
            ...
            selectCheck: "icons8-checked-checkbox",
            /**
        ...
        labels: {
            ...
            // infos: "共{{ctx.total}}条，当前显示 {{ctx.start}} 到 {{ctx.end}} 条",
            infos: "共{{ctx.total}}项记录",
            loading: "加载中...",
            noResults: "没有找到相关结果!",
            refresh: "刷新",
            search: "搜索"
        ...
        templates: {
            ...
            footer: "<div id=\"{{ctx.id}}\" class=\"{{css.footer}}\"><div class=\"row\"><div class=\"col-sm-6 infoBar\"><p class=\"{{css.infos}}\"></p><p class=\"{{css.actions}}\"></p></div><div class=\"col-sm-6\"><p class=\"{{css.pagination}}\"></p></div></div></div>",
            ...
            select: "<label class=\"{{css.selectLabel}}\"><input name=\"select\" type=\"{{ctx.type}}\" class=\"{{css.selectBox}}\" value=\"{{ctx.value}}\" {{ctx.checked}} /><span class=\"{{css.selectBox}}\"><i class=\"{{css.selectCheck}}\"></i></span></label>"
        }
    };
    ...
    Grid.prototype.select = function(rowIds)
    {
        if (this.selection)
        {
            ...
            if (selectedRows.length > 0)
            {
                ...
                this.element.find("thead input" + selectBoxSelector).prop("checked", selectMultiSelectBox);
                // 原先的代码有bug，按照如下修复
                if (!this.options.multiSelect)
                {
                     var lastId = this.selectedRows[this.selectedRows.length-1];
                     var checkboxes = this.element.find("tbody > tr input" + selectBoxSelector + ":checked");
                ...
                for (i = 0; i < this.selectedRows.length; i++)
                {
                    this.element.find("tbody > tr[data-row-id=\"" + this.selectedRows[i] + "\"]")
                        ...
                        .find("input" + selectBoxSelector).prop("checked", true);
                }
    ...
    Grid.prototype.deselect = function(rowIds)
    {
        if (this.selection)
        {
            ...
            if (deselectedRows.length > 0)
            {
                ...
                this.element.find("thead input" + selectBoxSelector).prop("checked", false);
                for (i = 0; i < deselectedRows.length; i++)
                {
                    this.element.find("tbody > tr[data-row-id=\"" + deselectedRows[i][this.identifier] + "\"]")
                        .removeClass(this.options.css.selected)._bgAria("selected", "false")
                        .find("input" + selectBoxSelector).prop("checked", false);
                }
